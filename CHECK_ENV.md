# تعليمات إصلاح Railway - مهم جداً! 🚨

## المشكلة:
Railway لا يجد متغيرات البيئة المطلوبة، خاصة:
1. `GOOGLE_SERVICE_ACCOUNT_BASE64` - مفتاح Google Service Account
2. `RESEND_API_KEY` - مفتاح البريد الإلكتروني (اختياري)

## الحل - اتبع هذه الخطوات بدقة:

### 1. افتح Railway Dashboard
اذهب إلى مشروعك في Railway

### 2. اذهب إلى Variables
- اضغط على Service الخاص بك
- اذهب إلى تبويب **Variables**
- اضغط على **RAW Editor**

### 3. انسخ هذه المتغيرات بالكامل والصقها:

```
GOOGLE_SHEETS_ID=1KWqy1Rw-1jIBOK0lQrBfqzQlZKzZN6GrKl4BF2LyOKw
GOOGLE_SERVICE_ACCOUNT_BASE64=
SESSION_SECRET=qortoba-supplies-2025-secure-session-key-xKj9Lm2Nq8
DEEPSEEK_API_KEY=sk-11817199e686483081e5d3ab5c679eb8
TELEGRAM_BOT_TOKEN=7910604598:AAGRxzJyg1UkRh_-aJK2vP1dWxRSo-Z7qlc
PORT=8080
```

### 4. احفظ المتغيرات
اضغط على **"Save"** أو **"Update"**

### 5. انتظر إعادة التشغيل
Railway سيعيد تشغيل التطبيق تلقائياً

## ⚠️ تنبيهات مهمة:

1. **GOOGLE_SERVICE_ACCOUNT_BASE64** مطلوب جداً - بدونه لن يعمل النظام
2. **SESSION_SECRET** مطلوب للأمان
3. **RESEND_API_KEY** اختياري - البريد الإلكتروني سيكون معطل بدونه (وهذا عادي)
4. **DATABASE_URL** اختياري - النظام يستخدم Google Sheets

## للتحقق من نجاح التشغيل:

بعد إضافة المتغيرات، يجب أن ترى في Railway logs:
- ✅ تم تحميل مفتاح Google من متغير البيئة
- ✅ تم تهيئة Google Sheets
- ⚠️ لا يوجد مفتاح Resend API - سيتم تعطيل إرسال البريد الإلكتروني (هذا عادي)

## إذا استمرت المشكلة:

1. تأكد من نسخ المتغيرات بالكامل
2. تأكد من عدم وجود مسافات زائدة
3. جرب حذف المتغيرات وإضافتها من جديد
4. ادفع آخر التغييرات إلى GitHub:
```bash
git add .
git commit -m "Fix environment variables for Railway"
git push
```

النظام سيعمل بنجاح بعد إضافة هذه المتغيرات! 🚀