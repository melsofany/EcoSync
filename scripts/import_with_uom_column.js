  ...
  try {
    ...
    for (let i = 0; i < excelData.length && i < 5449; i++) {
      ...
      try {
        ...
        // استخراج البيانات مع عمود الوحدة
        ...
        // إدراج البند مع الوحدة
        const itemResult = await sql`
          INSERT INTO items (
            id, item_number, description, unit, category, created_by
          ) VALUES (
            ${nanoid()}, ${itemNumber}, ${description || 'بند غير محدد'}, 
            ${unitOfMeasure || 'Piece'}, 'مستورد', 
        ...
        createdItems++;
        console.log(`📦 تم إنشاء البند: ${itemNumber} - الوحدة: ${unitOfMeasure || 'Piece'}`);
        ...
        if (rfqNumber) {
          ...
          await sql`
            INSERT INTO quotation_requests (
              ...
              'pending', 
              (SELECT id FROM users WHERE username = 'admin' LIMIT 1),
              ${`استيراد من Excel - الصف ${i + 1} - الوحدة: ${unitOfMeasure || 'Piece'}`}
            )
          `;
          ...
          if (poNumber) {
            ...
            await sql`
              INSERT INTO purchase_orders (
                ...
                (SELECT id FROM users WHERE username = 'admin' LIMIT 1),
                ${`مرتبط بطلب التسعير ${rfqNumber} - الصف ${i + 1} - الوحدة: ${unitOfMeasure || 'Piece'}`}
              )
            `;
