        ...
        <div className="bg-white rounded-lg border p-4 mt-4">
          ...
          <div className="overflow-x-auto">
            <Table className="text-sm">
              ...
              <TableBody>
                {/* البيانات التاريخية من قاعدة البيانات */}
                {historicalPricing && historicalPricing.length > 0 ? (
                  historicalPricing.map((pricing: any, index: number) => (
                    <TableRow key={`historical-${index}`} className="hover:bg-green-50 border-b">
                      {/* PROCESS NO */}
                      <TableCell className="text-center border font-bold">10500</TableCell>
                      {/* QUANTITY */}
                      <TableCell className="text-center border">{pricing.quantity || "1"}</TableCell>
                      {/* DATE/PO */}
                      ...
                      <TableCell className="text-center border font-bold text-green-600">
                        {pricing.unitPrice ? formatCurrency(Number(pricing.unitPrice), pricing.currency) : "-"}
                      </TableCell>
                      {/* QTY */}
                      <TableCell className="text-center border">{pricing.quantity || "1"}</TableCell>
                      {/* DATE */}
                      ...
                      <TableCell className="text-center border font-bold text-orange-600">
                        {pricing.requestNumber || "28R00000001"}
                      </TableCell>
                      {/* DESCRIPTION */}
                      <TableCell className="text-left border px-2 text-xs max-w-xs">
                        <div className="break-words">
                          {pricing.description?.toUpperCase() || item.item?.description?.toUpperCase()}
                        </div>
                      </TableCell>
                      {/* PART NO */}
                  ...
                  /* إذا لم توجد بيانات تاريخية، اعرض البيانات الحالية */
                  <TableRow className="border-b">
                    ...
                    <TableCell className="text-center border font-bold text-green-600">
                      {detailedPricing?.supplierPricings?.[0]?.unitPrice 
                    ...
                    <TableCell className="text-center border font-bold text-orange-600">28R00000001</TableCell>
                    <TableCell className="text-left border px-2 text-xs max-w-xs">
                      <div className="break-words">
                        {item.item?.description?.toUpperCase() || "CONTACTOR 32 AMP"}
                      </div>
                    </TableCell>
                    <TableCell className="text-center border font-bold">
                      {item.item?.partNumber || item.item?.itemNumber}
                    </TableCell>
                    <TableCell className="text-center border font-bold text-xs text-blue-600">
                      {item.item?.lineItem}
                    </TableCell>
