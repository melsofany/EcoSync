...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/users/:userId", requireAuth, requirePermission("edit_users"), async (req: Request, res: Response) => {
    try {
      ...
      } else if (updateData.hasOwnProperty('permissions')) {
        await logActivity(req, "update_user_permissions", "user", userId, 
          `تم تحديث صلاحيات المستخدم ${updatedUser.fullName}`);
      } else {
  ...
  // Delete user
  app.delete("/api/users/:userId", requireAuth, requirePermission("delete_users"), async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
  ...
  app.get("/api/admin/database-export", requireAuth, requireRole(["it_admin"]), async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        sqlDump += `-- =====================\n`;
        sqlDump += `INSERT INTO items (id, item_number, part_number, description, unit, created_at, created_by, category) VALUES\n`;
        ...
        if (supplierPricing.length > 0) {
          ...
          const supplierPricingValues = supplierPricing.map(pricing => {
            const values = [
              ...
              pricing.minimumOrderQuantity || 'NULL',
    ...
    try {
      ...
      res.json({
        ...
        requiredFields: [
          ...
          { field: 'client', label: 'اسم العميل', description: 'اسم الشركة أو العميل', required: true },
    ...
    try {
      ...
      const mappedData = filteredData.map((row: any, index: number) => {
        ...
        const result = {
          ...
          client: processedData.client || 'غير محدد',
    ...
    try {
      ...
      for (const row of previewData) {
        try {
          ...
          let client = await storage.getClientByName(row.client);
          if (!client && row.client) {
            const newClient = await storage.createClient({
              name: row.client,
              email: `${row.client.toLowerCase().replace(/\s+/g, '')}@example.com`,
              phone: '',
              address: ''
            });
            client = newClient;
