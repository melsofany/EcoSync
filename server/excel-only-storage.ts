...
export class ExcelOnlyStorage {
  ...
  async getAllQuotationRequestsWithClients() {
    console.log(`📋 عرض ${this.excelData.quotationRequests.length} طلب تسعير مع العملاء من البيانات المحفوظة`);
    return this.excelData.quotationRequests.map(quotation => ({
      ...quotation,
      clientName: quotation.clientName || 'عميل من البيانات المحفوظة'
    }));
  }
