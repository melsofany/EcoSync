// اختبار كتابة المعرفات الفريدة في Google Sheets
import { GoogleAuth } from 'google-auth-library';
import { google } from 'googleapis';

async function testWriteUniqueIds() {
  try {
    console.log('🔑 بدء كتابة المعرفات الفريدة في Google Sheets...');
    
    // بيانات المصادقة المحدثة
    const credentials = {
      type: "service_account",
      project_id: "cortoba-supp-sys",
      private_key_id: "75c0919d127eca6b97f7beece4b9f5c2b9bb5ba8",
      private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
      client_email: "cortoba-sys@cortoba-supp-sys.iam.gserviceaccount.com",
      client_id: "108486641505877917440",
      auth_uri: "https://accounts.google.com/o/oauth2/auth",
      token_uri: "https://oauth2.googleapis.com/token",
      auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
      client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/cortoba-sys%40cortoba-supp-sys.iam.gserviceaccount.com",
      universe_domain: "googleapis.com"
    };

    const auth = new GoogleAuth({
      credentials: credentials,
      scopes: ['https://www.googleapis.com/auth/spreadsheets']
    });

    const sheets = google.sheets({ version: 'v4', auth });
    const spreadsheetId = '1TuNmhUQSLCIJjyPKRGEX5WwCIlwgePdN5kBLkPSNGqg';

    console.log('📊 قراءة البيانات الحالية...');
    
    // قراءة البيانات الحالية من الشيت الأساسي
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: spreadsheetId,
      range: 'Sheet1!A:E'  // استخدام اسم الشيت الافتراضي
    });

    const rows = response.data.values || [];
    console.log(`📋 تم العثور على ${rows.length} صف`);

    // إنشاء عمود المعرفات الفريدة (العمود A فقط)
    const uniqueIds = rows.map((row, index) => {
      if (index === 0) {
        return ['معرف الصنف']; // العنوان
      } else {
        return [`P-${index.toString().padStart(7, '0')}`]; // P-0000001, P-0000002, etc.
      }
    });

    console.log(`📝 كتابة ${uniqueIds.length - 1} معرف فريد في العمود A...`);

    // كتابة العمود A فقط
    await sheets.spreadsheets.values.update({
      spreadsheetId: spreadsheetId,
      range: 'Sheet1!A:A',  // استخدام اسم الشيت الافتراضي
      valueInputOption: 'RAW',
      requestBody: {
        values: uniqueIds
      }
    });

    console.log('✅ تم كتابة المعرفات الفريدة بنجاح في Google Sheets!');
    console.log(`🆔 المعرفات من P-0000001 إلى P-${(uniqueIds.length - 1).toString().padStart(7, '0')}`);
    
    return {
      success: true,
      totalIds: uniqueIds.length - 1,
      firstId: 'P-0000001',
      lastId: `P-${(uniqueIds.length - 1).toString().padStart(7, '0')}`
    };

  } catch (error) {
    console.error('❌ خطأ في كتابة المعرفات:', error);
    return {
      success: false,
      error: error.message
    };
  }
}

// تشغيل الاختبار
testWriteUniqueIds().then(result => {
  if (result.success) {
    console.log(`🎉 نجح! تم إنشاء ${result.totalIds} معرف فريد`);
  } else {
    console.log(`❌ فشل: ${result.error}`);
  }
}).catch(error => {
  console.error('❌ خطأ عام:', error);
});